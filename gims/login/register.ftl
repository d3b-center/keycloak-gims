...
${msg("registerTitle")}
<#elseif section = "form">
<form id="kc-register-form" class="ui large form" action="${url.registrationAction}" method="post">
    ...
    <div class="mdc-text-field mdc-text-field--with-leading-icon ${properties.kcLabelClass!} <#if usernameEditDisabled??>mdc-text-field--disabled</#if>">
      ...
      <div class="field <#if usernameEditDisabled??>disabled</#if>">
        ...
        <div class="ui left icon input">
          <i class="user icon"></i>
          <input tabindex="0" required id="username" type="text" name="username" value="${(login.username!'')}" placeholder="Username or email" autofocus autocomplete="off">
        </div>
      </div>
